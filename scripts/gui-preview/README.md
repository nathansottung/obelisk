...
## Local comparison package candidate — 2026-09-21

The new unsigned package includes the accepted multi-snapshot and recorded-comparison runtime. It remains an uncommitted packaging candidate awaiting substantive review, separate from the accepted inventory ZIP. From its extracted directory, use `Launch.ps1 generate-pair ABS_NEW_WORKSPACE`, native `inventory` for each generated side, and `Launch.ps1 view ABS_ALPHA_CATALOG ABS_BETA_CATALOG`. Choose the comparison reference/root alignment explicitly. Type stop and Enter, wait for both readers and the packaged child, then reopen the same inputs. The single-catalog and intentional static commands remain supported. See the [quickstart](../windows-alpha/QUICKSTART.md), [supported limits](../windows-alpha/SUPPORTED.md), and [current package report](../../docs/development/reviews/WINDOWS-COMPARISON-ALPHA-PACKAGE-IMPLEMENTATION-2026-09-21.md). Windows x64, Node 24 x64, PowerShell 5.1 and a browser are required; no checkout or Go/Git is required at runtime. Generated-data scope only; Prompt20 remains DEFERRED_BY_OWNER.

// Stored ZIP entries, UTF-8 names, no dependencies; explicit entry list only.
...
export function makeZip(entries) {
  ...
  assert.ok(entries.length < 65535);
  for (const { name, data } of entries) {
    assert.match(name, /^[a-zA-Z0-9_./-]+$/); assert.ok(!name.startsWith('/') && !name.split('/').includes('..'));
    const filename = Buffer.from(name), crc = crc32(data), header = Buffer.alloc(30), directory = Buffer.alloc(46);

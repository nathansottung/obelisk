...
for(const [name,pkg,catalog]of cases.filter(([name])=>!selectedCase||name===selectedCase)){
  ...
  try{
    server=spawn(process.execPath,[path.join(pkg,'launcher.mjs'),...(catalog?['view',catalog]:['static'])],{cwd:inputs.cwd,env,windowsHide:true,stdio:['pipe','pipe','pipe']});
    ...
    const result={name,package:pkg,catalog,cwd:inputs.cwd,checks,durationMs:Date.now()-started,failed,forcedBrowser,forcedServer,serverExit:server?.exitCode,browserExit:browser?.exitCode,serverText,serverError,browserText};summary.push(result);await fs.writeFile(path.join(evidence,'summary.json'),JSON.stringify(summary,null,2));
  }
